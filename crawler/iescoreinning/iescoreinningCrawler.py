...
class iescoreinningCrawler(CrawlerBase):

  def baseball(self):
    # 융기원 DB 정보 config로 정의 해서 넘기기 아래 정보대로 데이터 적재함
    self.table = 'KBO_IE_SCOREINNING'
    ...
    for s in sch :
      url = BASEURL + '/Live/IE_SCOREINNING?season_id='+str(self.crawlerData.season_id)+'&g_id='+str(s)

      #테스트용 유알엘
      #url = 'https://ktaijson.sports2i.com/Api/Live/IE_SCOREINNING?season_id=2018&g_id=20180324HHWO0'
      #url = 'https://ktaijson.sports2i.com/Api/Live/IE_SCOREINNING?season_id=2020&g_id='+str(s)
      ...
      try:
        ...
        if r.status_code == 200:
          ...
          for d in data['LIST']:
            ...
            DicElement['INNING'] = d['INNING']
            DicElement['SCORE'] = d['SCORE']
